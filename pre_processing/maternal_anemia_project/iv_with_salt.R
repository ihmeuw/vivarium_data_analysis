########################################
# SUPPLEMENTAL R CODE FOR MATERNAL 
# ANEMIA PROJECT FOR BMGF
# AUGUST 2020
# ALI BOWMAN
#########################################
...
anc_effect.l <- melt(anc_effect, id.vars = id_vars, variable.name = "draw", value.name = "anc_effect")
...
df.p <- merge(df.p, anc_effect.l, by = by_vars)
df.p[, mean := mean+anc_effect]

df.preg <- copy(df.p)
df.preg[, prev_pregnant := NULL]
df.preg[, anc_effect := NULL]
...
fwrite(df, "anemia_prev_iv_with_salt.csv")