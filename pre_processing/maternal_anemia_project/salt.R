...
df[, severe := ens_mv2prev(hgb_upper_severe, mean, variance, w = w) - ens_mv2prev(hgb_lower_severe, mean, variance, w = w)
   , by = 1:nrow(df)]
print("CALCULATING IV TARGET")
df[, iv_target := ens_mv2prev(hgb_upper_iv_target, mean, variance, w = w) - ens_mv2prev(hgb_lower_iv_target, mean, variance, w = w)
   , by = 1:nrow(df)]
#Anemic is the sum
df[, anemic := mild + moderate + severe]

sevs <- c("mild", "moderate", "severe", "anemic", "iv_target")
...
fwrite(df, "anemia_prev_salt.csv")